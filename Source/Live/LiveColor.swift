...
public class LiveColor: LiveValue, CustomStringConvertible {
    ...
    static let linearP3ToLinearSRGBMatrix: matrix_float3x3 = {
        let col1 = SIMD3<Float>([ 1.2249, -0.2247,  0])
        let col2 = SIMD3<Float>([-0.0420,  1.0419,  0])
        let col3 = SIMD3<Float>([-0.0197, -0.0786,  1.0979])
        return matrix_float3x3([col1, col2, col3])
    }()
    
    func sRGB(p3 c: LiveColor) -> LiveColor {
        let p3 = SIMD3<Float>(x: Float(c.r.cg), y: Float(c.g.cg), z: Float(c.b.cg))
...
extension SIMD3 {
    ...
    var gammaDecoded: SIMD3<Float> {
        ...
        return SIMD3<Float>(f(x as! Float), f(y as! Float), f(z as! Float))
